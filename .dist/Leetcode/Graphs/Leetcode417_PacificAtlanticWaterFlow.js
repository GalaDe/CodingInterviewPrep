...
exports.pacificAtlantic_BFS = exports.pacificAtlantic_DFS = void 0;
function pacificAtlantic_DFS(matrix) {
    const result = [];
    const pac = new Set();
    const atl = new Set();
...
exports.pacificAtlantic_DFS = pacificAtlantic_DFS;
;
...
function pacificAtlantic_BFS(matrix) {
    const LAST_ROW = matrix.length - 1;
    const LAST_COL = matrix[0].length - 1;
    const pacificBeachQueue = [];
    const atlanticBeachQueue = [];
...
exports.pacificAtlantic_BFS = pacificAtlantic_BFS;
//Uses Adjacent Matrix to track visited nodes
function bfs_usingAdjacentMatrix(matrix, queue) {
    const visited = new Array(matrix.length).fill(undefined).map(() => new Array(matrix[0].length).fill(false));
...
function bfs_usingSet(matrix, queue) {
    let visited = new Set();
    while (queue.length > 0) {
        const [y, x] = queue.shift();
        visited.add(`${y} ${x}`);
...
//-------------------------------------------------------------------------------------------------------//
class Cell {
    constructor(row, col, pacific, atlantic) {
        this.row = row;
        this.col = col;
...
**/
//# sourceMappingURL=Leetcode417_PacificAtlanticWaterFlow.js.map