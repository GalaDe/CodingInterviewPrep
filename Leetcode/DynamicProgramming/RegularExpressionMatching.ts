...
function isMatchRecursiveImpl1(s: string, p: string, ss = 0, ps = 0): boolean {
    if (p.length - ps <= 0) {
...
function isMatchRecursiveImpl2(s: string, p: string): boolean {
    // solution for lazy :-D
    // return new RegExp(`^${p}$`).test(s);
  
    if (!p || p.length == 0) return s.length == 0;
...
export function isMatchDynamicProgrammingImpl(s: string, p: string): boolean {
...
export function isMatchDynamicProgrammingImpl2(text: string, pattern: string): boolean {
  const memo: boolean[][] = new Array(text.length + 1)
  .fill(false)
  .map(() => 
