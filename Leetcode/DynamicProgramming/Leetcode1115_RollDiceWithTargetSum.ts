...
export function numRollsToTarget(n: number, k: number, target: number): number {
    ...
    let dp: number[][] = new Array(n + 1).fill(undefined).map(() => new Array(target + 1).fill(0));
...
export function numRollsToTarget_DP(d: number, f: number, target: number): number {

    if(d * f < target) return 0;

    let exp = Math.pow(10, 9) + 7;

    let dp: number[][] = new Array(d + 1).fill(undefined).map(() => new Array(target + 1).fill(0));
...
export function numRollsToTarget_Memoization(n: number, k: number, target: number): number {
  const memo = new Map<string, number>();
  return helper(n, target, k, memo) % MOD;
}

function helper (i: number, j: number, face: number, memo:  Map<string, number>) {
    if (i === 0) {
      return j === 0 ? 1 : 0;
    }
