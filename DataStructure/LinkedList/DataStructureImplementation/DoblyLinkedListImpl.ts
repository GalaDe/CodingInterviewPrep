...
export class DoublyLinkedList{
    ...
    add(val: any){
        if(this.isEmpty()) return this.addNodeFront(val);
        else return this.addNodeLast(val);
    }

    addNodeFront(val: any){
    ...
    addNodeAtPosition(val: any, position: number){
        if(this.isEmpty()) return this.addNodeFront(val);
        if(position > this.size || position < 1) throw new Error (`There is no such position. The liked list size is: ${this.size}`)
        if(position == this.size) return this.addNodeLast(val);
        let curr =  this.head;
        let tmp = new DoublyLinkedListNode(val);
        let count = 1;
        while(curr != null){
    ...
    removeNodeAtPosition(position: number){
