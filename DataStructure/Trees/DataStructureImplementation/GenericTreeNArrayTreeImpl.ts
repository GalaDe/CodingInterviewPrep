...
export class NArrayGenericTree{

    root: GTNode | null;


    addNodeAtParticularLocation(tree: GTNode, key: any, val: any){
        if(tree == null) return;

        let currentNode: GTNode = null;
        let queue = [];
        queue.push(tree);
    ...
    delete(tree: GTNode, key: any){
        ...
        while(queue.length > 0) {
            ...
            for(let i = 0; i < currentNode.children.length; i++){
                ...
                else queue.push(currentNode.children[i])
            }
            queue.shift();
        }  
    }

    deleteDeepestNode(tree: GTNode){
        let currentNode: GTNode = null;
        let temp = tree;
        let queue = [];
        queue.push(tree);
    ...
    printPreOrder(tree: GTNode){
        if(tree == null) return null;

        var i = 0;
		var temp: GTNode = null;
    ...
    printBFS_LevelOrder(tree: GTNode){
        if(tree == null) return;
    ...
    */
    ...
    calculateHeightOfTree_BFS(root: GTNode){
        if (root == null) 
        return 0;
    ...
    calculateHeightOfTree_BFS_2(root: GTNode){
        if (root == null) 
        return 0;
    ...
    */
    ...
    calculateHeightOfTreeDFSRecursive(root: GTNode){
        if (root == null) 
            return 0;
    ...
    */
    ...
    calculateSizeOfTree(tree: GTNode){
