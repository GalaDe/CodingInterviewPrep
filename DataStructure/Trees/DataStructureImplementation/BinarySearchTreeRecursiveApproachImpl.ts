...
export class BinarySearchTreeRec{
    ...
    //Recursive approach
    insertRecursive(val: any){
        this.root = this.insert(this.root, val);
        return this.root;
    }

    insert(root: BSTNode, val: any){
        if(root == null){
    ...
    */        
    ...
    removeNode(current: BSTNode, value: any) {
    ...
    findMinValRecursive(root: BSTNode){
        if(root == null)
            return root;
    ...
    //Case 2: Helper method for delete the node with two children
    findMaxValRecursive(root: BSTNode){
        if(root == null)
            return;
    ...
    calculateHeightOfTree(root: BSTNode){
    ...
    calculateSizeOfTree(root: BSTNode){
        if(root == null) return 0;
        else return this.calculateSizeOfTree(root.left) + this.calculateSizeOfTree(root.right) + 1;
    }
}